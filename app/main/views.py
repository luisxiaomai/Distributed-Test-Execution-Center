...
def records():
    ...
    if searchValue:
        ...
        recordList = Record.query.order_by(desc(sort_item)).all(
        ) if order_seq == "desc" else Record.query.order_by(asc(sort_item)).all()
    temp = []
...
def async_execute():
    ...
    record = Record(test_cases=','.join(str(case) for case in caseList), task_id=task.id,
                    start_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status="in_process")
...
def recordStatistics():
    recordStatistics = {}
    recordStatistics["success_count"] = Record.query.filter_by(
        status="SUCCESS").count()
    recordStatistics["failure_count"] = Record.query.filter_by(
